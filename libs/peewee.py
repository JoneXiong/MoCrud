...
class BaseModel(type):
    ...
    def __new__(cls, name, bases, attrs):
        ...
        if not cls._meta.db_table:
            ...
            if cls._meta.table_split=='day':
                m_now = datetime.datetime.now()
                if not cls._meta.table_next:
                    m_fix = m_now.strftime('%Y%m%d')
                else:
                    m_time = m_now + datetime.timedelta(days = 1)
                    m_fix = m_time.strftime('%Y%m%d')
                cls._meta.db_table = '%s_%s'%(cls._meta.db_table, m_fix)
            elif cls._meta.table_split=='month':
                m_now = datetime.datetime.now()
                if not cls._meta.table_next:
                    m_fix = m_now.strftime('%Y%m')
                else:
                    last_day = calendar.monthrange(m_now.year, m_now.month)[1]
                    m_time = datetime.datetime(m_now.year, m_now.month,last_day) + datetime.timedelta(days = 1)
                    m_fix = m_time.strftime('%Y%m')
                cls._meta.db_table = '%s_%s'%(cls._meta.db_table, m_fix)
